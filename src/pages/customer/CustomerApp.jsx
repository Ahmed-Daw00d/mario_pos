...
function MenuTab() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto scrollbar-none border-b border-border bg-background flex-shrink-0">
        {categories.map(cat => (
          <button key={cat.id} onClick={() => setActiveCat(cat.id)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all flex-shrink-0
              ${activeCat === cat.id
                ...
                : 'text-foreground opacity-50 hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/10'}`}>
            <span>{cat.emoji}</span>
            <span>{cat.label}</span>
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          <button key={pizza.id} onClick={() => pizza.is_available && setSelectedPizza(pizza)}
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1">
                  <h3 className="font-bold text-foreground text-base leading-tight">{pizza.name_it}</h3>
                  {pizza.description_it && (
                    <p className="text-xs opacity-50 mt-1.5 leading-relaxed italic">{pizza.description_it}</p>
                  )}
                    ...
                    {pizza.ingredients?.slice(0, 5).map(ingId => {
                      ...
                      return ing ? (
                        <span key={ingId} className={`text-xs px-2 py-0.5 rounded-full border
                          ${!ing.is_available ? 'border-red-500/40 text-red-500 dark:text-red-400 line-through' : 'border-border opacity-50'}`}>
                          {ing.name_it}
                        </span>
                      ) : null;
                    })}
                    {pizza.ingredients?.length > 5 && (
                      <span className="text-xs px-2 py-0.5 rounded-full border border-border opacity-30">+{pizza.ingredients.length - 5}</span>
                    )}
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-brand-gold font-bold text-base">€{pizza.base_price_tonda?.toFixed(2)}</p>
                  <p className="text-xs opacity-40">Tonda</p>
                  <p className="text-brand-gold font-semibold text-sm mt-1">€{pizza.base_price_teglia?.toFixed(2)}</p>
                  <p className="text-xs opacity-40">Teglia</p>
                </div>
              </div>
              {!pizza.is_available && (
                <div className="mt-2 text-xs text-red-500 dark:text-red-400 font-semibold">⛔ Esaurito</div>
              )}
            </div>
            <ChevronRight size={16} className="opacity-30 mt-1 flex-shrink-0" />
          </button>
        ))}
        ...
        {activeCat === 'kebab' && (
          <button onClick={() => setShowKebab(true)}
            ...
            <div className="flex-1">
              <h3 className="font-bold text-foreground text-lg">Kebab di Pollo</h3>
              <p className="text-sm opacity-50 italic mt-0.5">Pollo fresco alla griglia</p>
              <p className="text-xs opacity-40 mt-1">Scegli il pane, le verdure e le salse</p>
            </div>
            {/* <div className="text-right">
              <p className="text-brand-gold font-bold text-xl">€9.00</p>
              <ChevronRight size={18} className="opacity-40 ml-auto mt-1" />
            </div> */}
          </button>
        )}
        ...
        {activeCat === 'fritti' && fritti.map(item => (
          <button key={item.id}
            ...
            <div className="flex-1">
              <h3 className="font-bold text-foreground">{item.name_it}</h3>
              {item.description_it && <p className="text-xs opacity-40 italic mt-0.5">{item.description_it}</p>}
              {!item.is_available && <p className="text-xs text-red-500 dark:text-red-400 mt-0.5 font-semibold">⛔ Esaurito</p>}
            </div>
            <div className="text-right">
              <p className="text-brand-gold font-bold text-lg">€{item.price.toFixed(2)}</p>
              <p className="text-xs opacity-40">Aggiungi +</p>
            </div>
          </button>
        ))}
        ...
        {activeCat === 'bevande' && (
          <div className="mb-2">
            <h4 className="text-sm font-semibold opacity-40 px-1 mb-2">🥤 Bevande</h4>
            {bevande.map(item => (
              <button key={item.id}
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground text-sm">{item.name_it}</p>
                </div>
...
function OrdersTab() {
  ...
  if (loading) return (
    <div className="flex-1 flex items-center justify-center opacity-40">Caricamento...</div>
  );

  if (!orders.length) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8 py-16">
      <span className="text-6xl">🍕</span>
      <p className="opacity-60 font-semibold text-lg">Nessun ordine ancora</p>
      <p className="opacity-30 text-sm">Inizia sfogliando il menu!</p>
    </div>
  );
...
function CartTab({ onOrderPlaced }) {
  ...
  if (cart.items.length === 0) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-8 py-16">
      ...
      <p className="opacity-60 font-semibold text-lg">Il carrello è vuoto</p>
      <p className="opacity-30 text-sm">Aggiungi qualcosa dal menu</p>
    </div>
  );
  ...
  if (submitted) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8 text-center">
      ...
      <h3 className="text-xl font-bold text-foreground">Ordine inviato!</h3>
      <p className="opacity-50 text-sm">La cucina ha ricevuto il tuo ordine</p>
    </div>
  );
    ...
    <div className="flex flex-col h-full">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                {item.removed_ingredients?.length > 0 && (
                  <p className="text-xs text-red-500 dark:text-red-400 mt-1">❌ Senza: {item.removed_ingredients.map(id => ingredients[id]?.name_it || id).join(', ')}</p>
                )}
                {item.added_ingredients?.length > 0 && (
                  <p className="text-xs text-green-600 dark:text-green-400">➕ Extra: {item.added_ingredients.map(id => ingredients[id]?.name_it || id).join(', ')}</p>
                )}
                {item.kebab_config && (
                  <div className="text-xs opacity-40 space-y-0.5 mt-1">
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => updateQty(item.cartId, item.quantity - 1)}
                  className="w-7 h-7 rounded-lg border border-border flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/10 transition-colors text-sm text-foreground">−</button>
                <span className="text-sm font-bold w-5 text-center text-foreground">{item.quantity}</span>
                <button onClick={() => updateQty(item.cartId, item.quantity + 1)}
                  className="w-7 h-7 rounded-lg bg-brand-red flex items-center justify-center hover:bg-red-700 transition-colors text-sm text-white">+</button>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="border-t border-border p-4 bg-background/95 backdrop-blur-sm">
        <div className="flex justify-between items-center mb-4">
          <span className="opacity-60 font-medium">Totale</span>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
      <div className="text-center mb-6">
        ...
        <p className="opacity-50 text-sm mt-1">Tavolo {session?.table_number}</p>
      </div>

      {orders.length === 0 && (
        <p className="text-center opacity-30 italic py-8">Nessun ordine ancora</p>
      )}

      {orders.map(order => (
        <div key={order.id} className="card p-4 space-y-2">
          <p className="text-xs opacity-40 uppercase tracking-wide">Ordine #{order.id.slice(-4).toUpperCase()}</p>
          {order.items?.map((item, i) => (
            <div key={i} className="flex justify-between text-sm">
              <span className="opacity-80">{item.quantity}× {item.item_name}
                {item.size && <span className="opacity-40 ml-1 capitalize">({item.size})</span>}
              </span>
              <span className="text-foreground font-medium">€{(item.total_price * item.quantity).toFixed(2)}</span>
            </div>
          ))}
      ...
      <div className="card p-5 text-center space-y-2 mt-4">
...
function CustomerAppInner() {
  ...
  return (
    <div className="h-screen bg-background flex flex-col max-w-md mx-auto relative overflow-hidden shadow-2xl">
      ...
      {/* Bottom Nav */}
      <nav className="sticky bottom-0 bg-background/95 backdrop-blur-md border-t border-border grid grid-cols-4 z-30">
        {tabs.map(tab => {
          const Icon = tab.icon;
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`relative flex flex-col items-center gap-1 py-3 transition-all
                ${isActive ? 'text-brand-red' : 'opacity-40 hover:opacity-70 text-foreground'}`}>
