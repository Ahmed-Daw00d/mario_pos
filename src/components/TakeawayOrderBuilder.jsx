    ...
    >
      ...
      <div className="bg-background border-b border-border px-5 py-4 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <ShoppingBag size={20} className="text-orange-500 dark:text-orange-400" />
          <div>
            <p className="font-bold text-foreground">🛍️ {session.customer_name}</p>
            {session.phone && <p className="text-xs opacity-40">{session.phone}</p>}
          </div>
        </div>
      ...
      {submitted ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <h3 className="text-xl font-bold text-foreground">Ordine Inviato in Cucina!</h3>
          <p className="opacity-50">🛍️ {session.customer_name}</p>
        </div>
      ) : (
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-h-0 border-r border-border">
            {/* Category tabs */}
            <div className="flex gap-2 px-3 py-3 overflow-x-auto scrollbar-none border-b border-border flex-shrink-0">
              {categories.map(c => (
                <button key={c.id} onClick={() => setActiveCat(c.id)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap flex-shrink-0 transition-all
                    ${activeCat === c.id ? 'bg-orange-500 text-white' : 'bg-black/10 dark:bg-white/10 opacity-50 hover:bg-black/20 dark:hover:bg-white/20'}`}>
                  {c.emoji} {c.label}
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
              ...
              {activeCat === 'pizza' && pizzas.filter(p => p.is_available !== false).map(pizza => (
                <button key={pizza.id}
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-foreground text-sm">{pizza.name_it}</p>
                    <p className="text-xs text-brand-gold">
                      Tonda €{pizza.base_price_tonda?.toFixed(2)}
                      {pizza.base_price_teglia && ` · Teglia €${pizza.base_price_teglia?.toFixed(2)}`}
                    </p>
                    <p className="text-xs opacity-30 italic mt-0.5">
              ...
              {activeCat === 'kebab' && (
                <button
                  ...
                  <div>
                    <p className="font-semibold text-foreground">Kebab di Pollo</p>
    ...
    <div className="w-72 flex flex-col min-h-0">
      ...
      {cart.items.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <p className="opacity-30 text-sm text-center italic px-4">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
          {cart.items.map(item => (
            <div key={item.cartId} className="bg-black/5 dark:bg-white/5 rounded-xl p-3">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-foreground leading-tight">{item.item_name}</p>
                  {item.size && <p className="text-xs opacity-40 capitalize">{item.size}</p>}
                  {item.split_type && item.split_type !== 'whole' && item.splits && (
                    <div className="mt-1 space-y-0.5">
                      {item.splits.slice(0, item.split_type === 'half' ? 2 : 3).map((s, si) => (
                        <p key={si} className="text-xs opacity-40">Q{si+1}: {s.pizza_name}</p>
                      ))}
                    </div>
                  )}
                  {item.removed_ingredients?.length > 0 && (
                    <p className="text-xs text-red-500 dark:text-red-400 mt-0.5">❌ {item.removed_ingredients.map(id => ingredients[id]?.name_it || id).join(', ')}</p>
                  )}
                  {item.added_ingredients?.length > 0 && (
                    <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">➕ {item.added_ingredients.map(id => ingredients[id]?.name_it || id).join(', ')}</p>
                  )}
                  {item.kebab_config && (
                    <p className="text-xs opacity-40 mt-0.5">🥙 {item.kebab_config.serving_style}</p>
                  )}
                </div>
                <button onClick={() => onRemove(item.cartId)}>
                  <Trash2 size={14} className="text-red-500 dark:text-red-400 flex-shrink-0" />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-6 h-6 rounded-lg border border-border flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10">
      ...
      <div className="border-t border-border p-4 space-y-3 flex-shrink-0">
        <div className="flex justify-between items-center">
          <span className="opacity-60 text-sm">Totale</span>
